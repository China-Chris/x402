...
cd "$PROJECT_ROOT/examples/typescript/servers/express"
echo "📡 启动 Facilitator (后台运行)..."
npx tsx index.ts > /tmp/facilitator.log 2>&1 &
FACILITATOR_PID=$!
...
for i in {1..20}; do
    if curl -s http://localhost:3000/facilitator/supported > /dev/null 2>&1; then
        echo "✅ Facilitator 运行中"
        break
    fi
    if [ $i -eq 20 ]; then
